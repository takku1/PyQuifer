...
class CognitiveCycle(nn.Module):
    ...
    def __init__(self, config: CycleConfig = None):
        ...
        if c.use_ou_dephasing:
            from pyquifer.dynamics.stochastic import OrnsteinUhlenbeckNoise
        ...
        from pyquifer.dynamics.criticality.controllers import CriticalityController
        ...
        from pyquifer.dynamics.metastability import MetastabilityIndex
        ...
        from pyquifer.dynamics.neuromodulation import NeuromodulatorDynamics
        self.neuromodulation = NeuromodulatorDynamics()

        # === Layer 4: Information Flow ===
        from pyquifer.cognition.reasoning.causal_flow import DominanceDetector
        self.dominance = DominanceDetector(
            num_levels=len(c.hierarchy_dims),
            buffer_size=200,
        )

        from pyquifer.dynamics.stochastic import AdaptiveStochasticResonance
        ...
        # === Layer 1: Learning ===
        from pyquifer.identity.neural_darwinism import SelectionArena, SymbiogenesisDetector
        ...
        from pyquifer.cognition.control.volatility import VolatilityGatedLearning
        ...
        if c.use_stp:
            from pyquifer.learning.stp import TsodyksMarkramSynapse
            self._stp = TsodyksMarkramSynapse(num_synapses=c.hierarchy_dims[0])

        if c.use_mean_field:
            from pyquifer.dynamics.oscillators.mean_field import KuramotoDaidoMeanField
        ...
        if c.use_koopman:
            from pyquifer.dynamics.criticality.monitors import KoopmanBifurcationDetector
        ...
        if c.use_neural_mass:
            from pyquifer.dynamics.continuous.neural_mass import WilsonCowanNetwork
        ...
        if c.use_ep_training:
            from pyquifer.learning.equilibrium_prop import EquilibriumPropagationTrainer
        ...
        if c.use_oscillation_gated_plasticity:
            from pyquifer.learning.synaptic import OscillationGatedPlasticity
        ...
        if c.use_three_factor:
            from pyquifer.learning.synaptic import ThreeFactorRule
        ...
        if c.use_oscillatory_predictive:
            from pyquifer.cognition.predictive.hierarchical import OscillatoryPredictiveCoding
        ...
        if c.use_sleep_consolidation:
            from pyquifer.learning.consolidation import SleepReplayConsolidation
        ...
        if c.use_dendritic_credit:
            from pyquifer.learning.dendritic import DendriticStack
        ...
        if c.use_no_progress:
            from pyquifer.dynamics.criticality.controllers import NoProgressDetector
            self._no_progress = NoProgressDetector(window_size=30)

        if c.use_evidence_aggregator:
            from pyquifer.cognition.control.metacognitive import EvidenceAggregator
            self._evidence_aggregator = EvidenceAggregator()

        if c.use_phase_cache:
            from pyquifer.dynamics.oscillators.mean_field import PhaseTopologyCache
        ...
        if c.use_workspace_ensemble:
            from pyquifer.workspace.ensemble import WorkspaceEnsemble
        ...
        if c.use_circadian:
            from pyquifer.embodiment.ecology import ChronobiologicalSystem
            self._circadian = ChronobiologicalSystem(dim=c.state_dim)

        if c.use_personality_attractor:
            from pyquifer.identity.strange_attractor import PersonalityAttractor
        ...
        if c.use_gating_loop:
            from pyquifer.dynamics.basal_ganglia import BasalGangliaLoop
        ...
        if c.solver != "euler":
            from pyquifer.dynamics.continuous.ode_solvers import SolverConfig
        ...
        if c.use_complex_oscillators:
            from pyquifer.dynamics.oscillators.complex import ComplexKuramotoBank
        ...
        if c.use_cuda_kernels:
            try:
                from pyquifer.accel.cuda.kuramoto_kernel import KuramotoCUDAKernel
        ...
        if c.use_visual_binding:
            from pyquifer.cognition.binding.visual import AKOrNEncoder
        ...
        if c.use_jepa_world_model:
            from pyquifer.cognition.predictive.jepa import ActionJEPA
        ...
        if c.use_deliberation:
            from pyquifer.cognition.control.deliberation import Deliberator
        ...
        if c.use_causal_reasoning:
            from pyquifer.cognition.reasoning.causal_reasoning import CausalGraph, DoOperator
        ...
        if c.use_appraisal:
            from pyquifer.cognition.control.appraisal import AppraisalChain, OCCModel
            self._appraisal = AppraisalChain(dim=c.state_dim)
            self._occ_model = OCCModel()

        # Phase 16: Selective SSM (oscillatory)
        self._selective_ssm = None
        if c.use_selective_ssm:
            from pyquifer.cognition.routing.ssm import OscillatorySSM
        ...
        if c.use_prospective_learning:
            from pyquifer.learning.prospective import InferThenModify
        ...
        if _want_cfc:
            from pyquifer.dynamics.oscillators.coupling import CrossFrequencyCoupling
    ...
    def register_organ(self, organ, adapter=None):
        ...
        if adapter is None and self.config.use_global_workspace:
            from pyquifer.workspace.organ_base import PreGWAdapter
    ...
    def tick(self,
        ...
        with torch.no_grad():
            ...
            if self.config.use_spike_activity:
                from pyquifer.dynamics.criticality.monitors import phase_activity_to_spikes
            ...
            _crit_fn = _cf_mod._compiled_criticality_feedback or _criticality_feedback
            _crit_fn(
        ...
        if c.use_phase_dominance:
            ...
            # Vectorized: compute band coherence for all levels at once
            complex_phases = torch.exp(1j * phases[:n_levels * band_size].to(torch.complex64))
            bands = complex_phases[:n_levels * band_size].reshape(n_levels, band_size)
            band_r = bands.mean(dim=1).abs()  # [n_levels]
            level_activations[:n_levels] = level_activations[:n_levels] * (0.5 + band_r)
        ...
        if self.config.use_global_workspace and self._organs:
            ...
            if gw_info.get('bg_stn_active', False):
                ...
                _BG_MODE_MAP = {0: 'perception', 1: 'imagination'}
                processing_mode = _BG_MODE_MAP.get(
                    gw_info['bg_mode_bias'], processing_mode
    ...
    def _compute_personality_blend(self, dominant_state: int,
        ...
        if attractor_state is not None:
            ...
            usable = num_pop * chunk
            if usable <= attractor_state.shape[0]:
                chunked = attractor_state[:usable].reshape(num_pop, chunk)
                biases = chunked.mean(dim=1).abs() * 0.3
                for i in range(num_pop):
                    weights[i] += biases[i].item()
    ...
    def compile_modules(self, mode: str = "default",
        ...
        """
        # _compiled_criticality_feedback lives in criticality_feedback module
        ...
        try:
            _cf_mod._compiled_criticality_feedback = torch.compile(
