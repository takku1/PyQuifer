...
class TransferEntropyEstimator(nn.Module):
    ...
    def compute_te(self,
        ...
        # Hash past k values into single index
        powers = K ** torch.arange(k, device=source.device)
        y_past_hash = torch.zeros(n_samples, dtype=torch.long, device=source.device)
        x_past_hash = torch.zeros(n_samples, dtype=torch.long, device=source.device)
        for i in range(k):
            y_past_hash += tgt_d[k - i: k - i + n_samples] * powers[i]
            x_past_hash += src_d[k - i: k - i + n_samples] * powers[i]
